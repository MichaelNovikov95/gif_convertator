...
export const convertVideoToGIF = (videoPath: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    ffmpeg.ffprobe(videoPath, (err, metadata) => {
      ...
      console.log('stream.duration', stream.duration);

      if (!stream || typeof stream.width !== 'number' || typeof stream.height !== 'number' || typeof stream.duration !== 'number') {
      ...
      if (width > 1024 || height > 768 || duration > 10) {
        return reject(new Error('Video must be 1024x768 or smaller and 10 seconds or less.'));
      }
